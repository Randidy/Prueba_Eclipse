...
public class BDCarrito {
	...
	private String URL = "jdbc:mysql://localhost:3306/tiendaexa2024";
	...
	public ArrayList<Categorias> ListarCategorias(){
		...
		try {
				...
				while(Rs.next()) {
					miListaC.add(new Categorias(Rs.getString("IdCategoria"),
	...
	public ArrayList<Productos> ListarProductos(String IdCat){
		...
		try {
				...
				Rs = Stmt.executeQuery();
				while(Rs.next()) {
					miListaP.add(new Productos(Rs.getString("IdProducto"),
    ...
    { boolean estado = false;
        String SQL = "SELECT * FROM Usuarios WHERE IdUsuario='"+
                    IdUsuario+"' AND Password='"+Password+"'";
        try {
    ...
    public ArrayList<Productos> ListarTodosLosProductos() {
        ...
        try {
            ...
            // Recorre el ResultSet y agrega los productos a la lista
            while (Rs.next()) {
                lista.add(new Productos(
                    ...
                    Rs.getInt("Stock"),
                    Rs.getString("Imagen"),
                    Rs.getString("Estado").charAt(0)  // Convierte el estado a char
                ));
            }
            Rs.close();
        } catch (SQLException e) {
